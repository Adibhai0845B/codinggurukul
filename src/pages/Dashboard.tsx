  ...
  return <div className="student-dashboard pb-16 animate-in fade-in duration-500">
    <header className="border-b border-border pb-8 pt-3">
      <div className="flex flex-col justify-between gap-8 lg:flex-row lg:items-end">
        <div><p className="flex items-center gap-3 text-xs font-bold uppercase tracking-[.22em] text-blue-400"><span className="h-px w-8 bg-orange-500" />Student workspace</p><h1 className="mt-5 text-4xl font-black tracking-[-.04em] text-white md:text-6xl">Good to see you,<br /><span className="text-blue-400">{username}.</span></h1><p className="mt-5 max-w-xl text-base leading-7 text-slate-400">We’ve picked today’s work from the topics that need your attention most.</p></div>
    ...
    <div className="grid lg:grid-cols-[1fr_340px]">
      <main id="today-plan" className="py-9 lg:border-r lg:border-white/10 lg:pr-10">
        ...
        <div>{studyPlan.length ? studyPlan.map((question, index) => <div key={question.id} className="group grid grid-cols-[2.5rem_1fr_auto] items-center gap-3 border-b border-white/10 py-5"><span className="font-mono text-xs text-slate-600">{String(index + 1).padStart(2, "0")}</span><div className="min-w-0"><p className="truncate font-bold text-slate-100">{question.title}</p><p className="mt-1 text-xs text-slate-500">{question.topic} <span className="mx-1 text-slate-700">/</span> {question.difficulty}</p></div><div className="flex items-center gap-2"><button onClick={() => completeProblem(question.id)} className="grid h-9 w-9 place-items-center text-slate-500 transition hover:text-emerald-400" aria-label={`Mark ${question.title} complete`}><Circle className="h-5 w-5" /></button><a href={problemUrl(question)} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-sm font-bold text-blue-400 transition hover:text-orange-400">Solve <ArrowRight className="h-4 w-4" /></a></div></div>) : <div className="border-b border-white/10 py-12 text-center text-slate-400"><CheckCircle2 className="mx-auto mb-3 h-7 w-7 text-emerald-400" />The entire DSA sheet is complete.</div>}</div>
