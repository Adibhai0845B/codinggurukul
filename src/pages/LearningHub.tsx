...
export default function LearningHub() {
  ...
  return (
    <div className="space-y-8 pb-10 animate-in fade-in duration-500">
      ...
      <Card>
        <CardHeader className="gap-5 md:flex-row md:items-end md:justify-between"><div><CardTitle className="flex items-center gap-2 text-2xl"><BriefcaseBusiness className="h-6 w-6 text-blue-500" />Company preparation</CardTitle><p className="mt-2 text-sm text-muted-foreground">Practise using the company tags in the Company Specific Sheet.</p></div><Select value={company} onValueChange={setCompany}><SelectTrigger className="w-full md:w-56"><SelectValue /></SelectTrigger><SelectContent>{featuredCompanies.map((item) => <SelectItem key={item} value={item}>{item}</SelectItem>)}</SelectContent></Select></CardHeader>
        <CardContent><div className="mb-5 flex items-center gap-4"><Progress value={companyQuestions.length ? (companyDone / companyQuestions.length) * 100 : 0} className="h-2 flex-1" /><span className="whitespace-nowrap text-sm font-bold">{companyDone}/{companyQuestions.length} solved</span></div><div className="grid gap-3 md:grid-cols-2">{companyQuestions.slice(0, 8).map((question) => <QuestionRow key={question.id} question={question} completed={completedIds.includes(question.id)} bookmarked={bookmarkedIds.includes(question.id)} onComplete={() => completeQuestion(question.id)} onBookmark={() => toggleBookmark(question.id)} />)}</div>{companyQuestions.length > 8 && <Button asChild variant="link" className="mt-4 px-0"><Link href="/company-specific">View all in Company Specific Sheet <ArrowRight className="ml-2 h-4 w-4" /></Link></Button>}</CardContent>
      </Card>
    </div>
  );
}
